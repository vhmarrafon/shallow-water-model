...
implicit none
integer,parameter::xt=240,yt=106,nt=15000
...
real::dy,Nc,vmed,umed,dhdx,dhdy,dudx,dudy,dvdx,dvdy,curvx,curvy,disx,disy,dnudx,dnvdy,dudx2,dudy2,dvdx2,dvdy2,mass
real,parameter::H=200,dlon=1.5,dlat=1.5,dt=200,rad=3.1416/180,g=9.81,lon0=300,lat0=-67.5,r_earth=6371000,r=0.0000001,nic=0.0015
integer::i,j,n
...
do j=1,yt	
	...
	!f_cor(j)=0    !tirando o parametro de coriolis
	write(*,*)lat(j)
...
do i=1,xt-1
	do j=1,yt-1
		!if(lat(j)<0)np(i,j)=exp(-1*((lon(i)-lon0)/10)**2-1*((lat(j)-lat0)/10)**2)
		!if(lat(j)>0)np(i,j)=-exp(-1*((lon(i)-lon0)/10)**2-1*((lat(j)+lat0)/10)**2)
