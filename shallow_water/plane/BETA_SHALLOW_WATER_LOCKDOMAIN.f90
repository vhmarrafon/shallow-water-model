...
! CODIGO PARA CALCULO DA EQUACAO DE AGUAS RASAS CONSIDERANDO UM DOMINIO FECHADO
! A DISCRETIZACAO FOI REALIZADA CONSIDERANDO UMA GRADE C
! ps:existe uma pequena assimetria no equador (motivo?)
!
...
do i=1,xt-1
	do j=1,yt-1
		np(i,j)=exp((-1.*((i-0.5*(xt-10))/5.)**2)-1.*(((j-0.5*yt)/5.)**2))
		!write(*,*)np(i,j)
	enddo
enddo
...
!! CONDICAO DE CONTORNO
			...
			elseif(j==yt .and. i/=1 .and. i/=xt)then
				vf(i,j)=0
				uf(i,j)=up(i,j)+c1*fb*(vp(i,j)+vp(i,1)+vp(i-1,1)+vp(i-1,j))-c2*(np(i,1)-np(i-1,1))-r*up(i,j)*dt
			...
			elseif(i==xt .and. j/=1 .and. j/=yt)then
				vf(i,j)=vp(i,j)-c1*fb*(up(i,j)+up(i,j-1)+up(1,j-1)+up(i+1,j))-c3*(np(1,j)-np(1,j-1))-r*vp(i,j)*dt
				uf(i,j)=0
...
!! CONDICAO DE CONTORNO
			...
			elseif(j==yt .and. i/=1 .and. i/=xt)then
				vf(i,j)=0
				uf(i,j)=ui(i,j)+2*c1*fb*(vp(i,j)+vp(i,1)+vp(i-1,1)+vp(i-1,j))-2*c2*(np(i,j)-np(i-1,j))-r*up(i,j)*2*dt
			elseif(i==1 .and. j/=1 .and. j/=yt)then
				vf(i,j)=vi(i,j)-2*c1*fb*(up(i,j)+up(i,j-1)+up(i+1,j-1)+up(i+1,j))-2*c3*(np(i,j)-np(i,j-1))-r*vp(i,j)*2*dt
				uf(i,j)=0
			elseif(i==xt .and. j/=1 .and. j/=yt)then
				vf(i,j)=vi(i,j)-2*c1*fb*(up(i,j)+up(i,j-1)+up(1,j-1)+up(1,j))-2*c3*(np(i,j)-np(i,j-1))-r*vp(i,j)*2*dt
				uf(i,j)=0
			elseif(i==1 .or. i==xt .and. j==1)then
				vf(i,j)=0
				uf(i,j)=0
